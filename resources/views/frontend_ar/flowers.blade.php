						...
						<div class="col-md-3">
							<div id="sidebar" class="sidebar">
								...
								<ul data-aos="fade-up">
									...
									<li><a
											href="{{url('ar/flowers/'.$flowerCategory->slug)}}">{{$flowerCategory->name_ar ?: $flowerCategory->name}}</a>
									</li>
									@endforeach
								</ul>
							</div>
...
<div class="modal fade" id="modal{{$flower->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel4"
	aria-hidden="true">
	<div class="modal-dialog modal-dialog-centered modal-dialog-zoom modal-lg" role="document">
		<div class="modal-content">
			<form onSubmit="addToCart(event,this)">
				{{csrf_field()}}
				<input type="hidden" name="flower" value="{{$flower->title_ar ?:$flower->title}}" />
				...
				<div class="row">
					...
					<div class="col-md-6 no-padding">
						<div class="pd_rgt">
							...
							<h4>{{$flower->title_ar ?:$flower->title}}
								@if(auth()->guard('customer')->check())
								<a href="javascript:void(0);" style="color:tomato"
									onclick="addFlowerToWishList('{{$flower->id}}','{{$flower->title_ar ?:$flower->title}}','{{asset($flower->getMedia('images')->first()->getUrl())}}')"
...
// 	$("#CartEmpty").html("")
// 	let appendData = sidebarSingleItem(data)
// 	console.log(appendData)
// 	$("#SidebarCartItems").append(appendData);
// }
...
function addFlowerToWishList(flowerId,name,image){
	$.ajax({
  type: "POST",
