...
<p>
    <ul class="list-inline">
        <li><a href="{{ route('admin.flowers.index') }}"
                style="{{ request('show_deleted') == 1 ? '' : 'font-weight: 700' }}">@lang('quickadmin.qa_all')</a></li>
        |
        <li><a href="{{ route('admin.flowers.index') }}?show_deleted=1"
...
<div class="panel panel-default">
    ...
    <div class="panel-body table-responsive">
        <table
            class="table table-bordered table-striped {{ count($flowers) > 0 ? 'datatable' : '' }} @can('flower_delete') @if ( request('show_deleted') != 1 ) dt-select @endif @endcan">
            <thead>
                <tr>
                    @can('flower_delete')
            ...
            <tbody>
                ...
                <tr data-entry-id="{{ $flower->id }}">
                    ...
                    <td field-key='category'>{{ $flower->category->name }} <br />{{ $flower->category->name_ar }}</td>
                    <td field-key='title'>{{ $flower->title }} <br />{{ $flower->title_ar }}</td>
