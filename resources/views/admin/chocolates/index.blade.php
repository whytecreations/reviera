...
<p>
    <ul class="list-inline">
        <li><a href="{{ route('admin.chocolates.index') }}"
                style="{{ request('show_deleted') == 1 ? '' : 'font-weight: 700' }}">@lang('quickadmin.qa_all')</a></li>
        |
        <li><a href="{{ route('admin.chocolates.index') }}?show_deleted=1"
...
<div class="panel panel-default">
    ...
    <div class="panel-body table-responsive">
        <table
            class="table table-bordered table-striped {{ count($chocolates) > 0 ? 'datatable' : '' }} @can('chocolate_delete') @if ( request('show_deleted') != 1 ) dt-select @endif @endcan">
            <thead>
                <tr>
                    @can('chocolate_delete')
            ...
            <tbody>
                ...
                <tr data-entry-id="{{ $chocolate->id }}">
                    ...
                    <td field-key='category'>{{ $chocolate->category->name }}<br />{{ $chocolate->category->name_ar }}
                    </td>
                    <td field-key='title'>{{ $chocolate->title }}<br />{{ $chocolate->title_ar }}</td>
